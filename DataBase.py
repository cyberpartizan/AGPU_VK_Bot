...
def insert_to_chats_t(peer_id, group_name):
...
def set_last_lessons_by_peer_id(peer_id,day_lessons):
    cursor.execute("UPDATE Chats SET last_lessons = ? WHERE chat_id =?",(day_lessons,peer_id))
    connection.commit()

def review_ChatsT(peer_id, group_name):
    if chat_id_is_in_DB(peer_id):
        update_to_chats_t(peer_id, group_name)
    else:
        insert_to_chats_t(peer_id, group_name)
