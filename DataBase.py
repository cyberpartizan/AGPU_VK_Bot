...
class Database:
    ...
    def chat_id_is_in_DB(self,chat_id):
        self.cursor.execute("SELECT EXISTS(SELECT chat_id FROM Chats WHERE chat_id=:chat_idKey)", {'chat_idKey': chat_id})
    ...
    def update_to_chats_t(self,peer_id, group_name):
        ...
        self.execute("UPDATE Chats SET group_link = ? WHERE chat_id =?", (group_link, peer_id))
        self.connection.commit()
